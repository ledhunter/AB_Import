...
public class StagedRow
{
    ...
    /// <summary>
    /// Журнал реальных действий, выполненных по этой строке в Apply-фазе:
    /// JSON-массив строк вроде <c>["Корпус создан", "Помещение обновлено",
    /// "ДДУ найден (не создан)"]</c>. Заполняется маппером (если маппер это
    /// поддерживает) и отдаётся в построчном отчёте, чтобы пользователь видел
    /// не только статус, но и что реально произошло с записью в Visary.
    /// <c>null</c> — маппер не сообщил per-row actions.
    /// </summary>
