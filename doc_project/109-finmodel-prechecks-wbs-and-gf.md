...
> ⚠️ **Поведение Pre-check 1 изменено в [doc 144 v1.1](./144-finmodel-rate-omitted-and-gf-needs-budget.md) (2026-06-22).**
> Раньше: WBS exists → `budgetUploadOk=true`, ГФ дополнял существующую ИСР.
> Теперь: WBS exists → `budgetUploadOk` остаётся `null`, ГФ тоже пропускается
> («Найден ИСР в Объекте — всё, импорт Бюджета и ГФ пропускаются»). Pre-check 2
> (skip существующего `CostItem`) остался без изменений. Описание ниже — историческое,
> актуальное поведение в doc 144 v1.1.
...
1. **Pre-check 1 (бюджет)** — перед заливкой XLSX через `BudgetVisaryUploader` проверяется, есть ли в ИСР выбранного `constructionsite` хотя бы один WBS-узел. Если есть — заливка пропускается. ~~ГФ продолжает работать с уже существующими статьями.~~ **Doc 144 v1.1: ГФ тоже пропускается.**
