...
**Статус**: 🟡 Фундамент готов (v0.1) — методы клиента + smoke-тесты;
встройка в `FinModelImportMapper` будет отдельным шагом.
**Дата**: 2026-05-07
...
Открытые вопросы для v0.2 (`FinModelImportMapper` поддержка бюджета):

- [ ] Парсинг секции «Себестоимость» на листе `Inputs` — индикатор начала бюджета.
- [ ] Парсинг блоков «Глава N» — каждый запускает группу подстатей.
- [ ] Маппинг "Title из файла" → Code (через справочник из `Context/Бюджет_А4.1...xlsx`).
- [ ] Чтение `DeclaredSum` / `ConfirmedSum` (по столбцу E согласно ТЗ пользователя).
- [ ] Идемпотентность: при повторном импорте — PATCH сумм у существующих записей,
  а не дублирование. Нужен метод `PatchWbsAsync` (не реализован).
- [ ] Какие главы создавать автоматически, а какие должны уже существовать
  (политика «жёстко требовать главы → ошибка, если их нет» против
  «автосоздавать недостающие»).
