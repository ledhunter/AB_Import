...
| [82-visary-file-storage-upload.md](./82-visary-file-storage-upload.md) | 📤 Автозагрузка XLSX-бюджета в файловое хранилище Visary + создание `typedimportwbs` (TypedJournal). 3 эндпоинта: `POST /api/files/files/upload` (multipart, field=`upload`) → `POST /api/files/link/file_link/by_id` (link-токен) → `POST /api/visary/crud/typedimportwbs` (`ImportType=10`, `File=<link>`). Новые: `IFileStorageClient`, `CreateTypedImportWbsAsync`, `BudgetVisaryUploader` (orchestrator), `BudgetUploadOptions` (`Visary:BudgetUpload:DriveId/DirectoryId/ImportType`), endpoint `POST /api/imports/{id}/budget-upload`. UI: 2-я кнопка в «Сформированные файлы» через `actionUrl` поле в `ApiGeneratedFile`. |
