...
| [78-budget-xlsx-export.md](./78-budget-xlsx-export.md) | 📤 **Заменяет [70](70-wbs-api-foundation.md)/[71](71-finmodel-budget-import.md):** CRUD-путь WBS отключён (Visary 500 на `listview/wbs/onetomany/ConstructionProject`). Бюджет «Финмодели» теперь выгружается XLSX по эталону `Бюджет_А4.1`: embedded resource → ClosedXML копия → подмена сумм в C/D, агрегация Глава/Раздел снизу вверх. `GET /api/imports/{id}/budget-xlsx`, кнопка «Скачать бюджет для Visary» в `SessionView` |
